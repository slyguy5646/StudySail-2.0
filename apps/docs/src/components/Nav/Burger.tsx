...
function BurgerOptions({
  ...
  return (
    ...
    >
      ...
      <Dialog.Panel
        className={`fixed inset-y-0 left-0 z-50 w-full overflow-y-auto border-r ${"border-r-slate-200 bg-background text-black"} px-4 py-6  sm:max-w-sm sm:ring-1 sm:ring-gray-900/10`}
      >
            ...
            <div className="space-y-2 px-6 py-6">
              ...
              {documents.map((document, index) => (
                <div onClick={() => setOpen(false)}>
                  <DocumentRow
                    key={index}
                    item={{ title: document.title, slug: document.title }}
                    router={router}
